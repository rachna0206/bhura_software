...
{
    $stmt_company_list = $this->con->prepare("SELECT r1.inq_id, COUNT(ta.user_id) AS assign_count, json_unquote(c1.raw_data->'$.post_fields.Firm_Name') as firm_name, json_unquote(c1.raw_data->'$.post_fields.Contact_Name') as contact_name, json_unquote(c1.raw_data->'$.post_fields.Mobile_No') as contact_no, CASE WHEN json_unquote(c1.raw_data->'$.post_fields.loan_applied') = 'Sactioned Loan' THEN 'Subsidy (Loan - Sactioned)' WHEN json_unquote(c1.raw_data->'$.post_fields.loan_applied') = 'Want to Apply?' THEN 'Loan - Want to Apply' WHEN json_unquote(c1.raw_data->'$.post_fields.loan_applied') = 'Loan Under Process' THEN 'Subsidy (Loan - Under Process)' ELSE 'Yet to decide' END AS vertical, us1.name AS current_user_name, (SELECT u1.name from tbl_tdrawassign r1, tbl_users u1 WHERE r1.user_id=u1.id and user_id!=31 and inq_id=r1.inq_id order by r1.id desc LIMIT 1) as forwarded_by, date_format(c1.raw_data_ts,'%d-%m-%Y') as received_on, COALESCE((SELECT date_format(cdate,'%d-%m-%Y') FROM tbl_tdrawdata_cdates WHERE inq_id = r1.inq_id ORDER BY id DESC LIMIT 1), '01-01-1970') as completion_date, json_unquote(c1.raw_data->'$.post_fields.Area') as area, CASE WHEN json_unquote(c1.raw_data->'$.post_fields.loan_applied') = 'Sactioned Loan' THEN COALESCE(NULLIF(json_unquote(c1.raw_data->'$.post_fields.TL_Amount'), ''), 0) WHEN json_unquote(c1.raw_data->'$.post_fields.loan_applied') = 'Want to Apply?' THEN COALESCE(NULLIF(json_unquote(c1.raw_data->'$.post_fields.Term_Loan_Amount') + json_unquote(c1.raw_data->'$.post_fields.CC_Loan_Amount'), ''), 0) WHEN json_unquote(c1.raw_data->'$.post_fields.loan_applied') = 'Loan Under Process' THEN COALESCE(NULLIF(json_unquote(c1.raw_data->'$.post_fields.Term_Loan_Amount_In_Process'), ''), 0) ELSE 0 END AS loan_amount FROM tbl_tdrawassign r1 JOIN (SELECT MAX(t2.id) AS r_id, t2.inq_id FROM tbl_tdrawassign t1 JOIN tbl_tdrawassign t2 ON t1.id = t2.id GROUP BY t2.inq_id) AS tbl1 ON tbl1.r_id = r1.id JOIN tbl_tdrawdata_cdates cd1 ON r1.inq_id = cd1.inq_id JOIN tbl_tdrawdata c1 ON r1.inq_id = c1.id JOIN tbl_users us1 ON r1.user_id=us1.id LEFT JOIN tbl_tdrawassign ta ON r1.inq_id = ta.inq_id AND ta.user_id = 31 WHERE r1.stage = 'lead' AND cd1.cdate < CURDATE() GROUP BY r1.inq_id HAVING assign_count > 0");
    $stmt_company_list->execute();
    $company_result = $stmt_company_list->get_result();
    $stmt_company_list->close();
    return $company_result;
