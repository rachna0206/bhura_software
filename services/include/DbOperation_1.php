...
//get plot no
public function get_plot_no($taluka,$ind_estate,$area)
{
...
//get plot floor
public function get_plot_floor($taluka,$industrial_estate,$area,$plot_no,$road_no)
{
...
//get road plot 
public function get_road_plot($taluka,$industrial_estate,$area)
{
...
public function plotno_for_roadwise($estate_id,$road_no)
{
    ...
    $res_plot=$this->get_road_plot($result_estate['taluka'],$result_estate['industrial_estate'],$result_estate['area_id']);
...
{
    $result_estate=$this->get_ind_estate($estate_id);
    $res_plot=$this->get_plot_no($result_estate['taluka'],$result_estate['industrial_estate'],$result_estate['area_id']);
    if(mysqli_num_rows($res_plot)>0)
    {
        while($plot = mysqli_fetch_array($res_plot))
        {
...
public function floorno_list($estate_id,$road_no,$plot_no)
{
    ...
    $res_plot=$this->get_plot_floor($result_estate['taluka'],$result_estate['industrial_estate'],$result_estate['area_id'],$plot_no,$road_no);
...
{
    $stmt_filter = $this->con->prepare("SELECT id,assign_estate_status FROM `pr_emp_estate` where industrial_estate_id=? and employee_id=?");
